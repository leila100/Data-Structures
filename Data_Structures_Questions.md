...
1. What is the runtime complexity of `insert`? O(log n) (worst case: O(n))

2. What is the runtime complexity of `contains`? O(log n) (worst case: O(n))

3. What is the runtime complexity of `get_max`? O(log n) (worst case: O(n))
...
10. What is the runtime complexity of `DoublyLinkedList.delete`? O(1)
    ...
    The doubly linked list's 'delete' would do better if you want to remove the first element (worst case runtime of Array.splice). The array splice would remove the first element, then have to re-index all the rest of the elements. The doubly linked list can do it in constant time.
